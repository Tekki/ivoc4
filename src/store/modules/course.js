    ...
    if (samplePattern.test(id)) {
      sample.get(id).then(
        response => {
          ...
          context.commit('minDay', 1)
          context.commit('metadata', [null, ['title', course.title], ['boxes', course.boxes]])

          const worksheetArray = Object.keys(course.worksheets).map(key => course.worksheets[key])
          context.commit('worksheets', worksheetArray)
          course.questions.forEach(
            question => {
              question.topic = course.worksheets[question.topic]
